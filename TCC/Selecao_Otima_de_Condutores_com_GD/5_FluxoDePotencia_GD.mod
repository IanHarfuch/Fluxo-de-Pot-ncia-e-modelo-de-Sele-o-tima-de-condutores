...
subject to BalancoPotenciaAtiva{i in Ob}:
	sum{(k,i) in Ol}(P[k,i]) - sum{(i,j) in Ol}( P[i,j] + L[i,j] * sum{c in Oc}( Rc[c]*Fi[i,j,c]) ) + PS[i] + PGD[i] = PD[i];

#Balanco de Potencia Reativa (6)
subject to BalancoPotenciaReativa{i in Ob}:
	sum{(k,i) in Ol}(Q[k,i]) - sum{(i,j) in Ol}( Q[i,j] + L[i,j] * sum{c in Oc}( Xc[c] *Fi[i,j,c]) ) + QS[i] + (PGD[i] * fp) = QD[i];
...
#==============================================================	
 ...
 #==============================================================
 	...
 	subject to ImplementacaoGD1{i in Ob}:
 		PGDmin * WGD[i] <= PGD[i];
 	
 	subject to ImplementacaoGD2{i in Ob}:
 	 	PGD[i] <= PGDmax * WGD[i];
 	 
 	subject to  ImplementacaoGD3{i in Ob}:
 		sum{c in Ob}(WGD[c]) = NGD;
